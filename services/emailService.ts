    ...
    try {
      if (supabase) {
        ...
        if (!error && data?.success) {
          console.log(`[Email Service] Delivered via default Resend transport (${data.id})`);
          await db.logAction(
            'EMAIL_SENT',
          ...
          console.warn('[Email Service] send-reports function returned error:', error || data?.error);
        }
      }
    } catch (edgeErr) {
      console.warn('[Email Service] Exception during send-reports invocation:', edgeErr);
    }

    // Step 3: Local Express Server Fallback (Calls /api/send-email)
    try {
  ...
  async sendMemberPurchaseEmail(member: any) {
    ...
    try {
      ...
      const html = buildBoxedEmailHtml({
        ...
        calloutBox: pdfBase64 ? `📄 <strong>Official Document Attached:</strong> ${member.guest_name || 'Member'}'s signed Membership Agreement & Facility Rules PDF is attached to this email (Membership_Agreement_${member.membership_number || 'Record'}.pdf).` : undefined,
        dataFields: [

...
import { emailService, buildBoxedEmailHtml } from './emailService';
            ...
            if (type === 'member_freeze') {
                ...
                html = buildBoxedEmailHtml({
                    ...
                    dataFields: [
                        ...
                        { label: 'CUSTOMER PROFILE', value: data.guest_name || 'Walk-in Guest' },
                        { label: 'VOID JUSTIFICATION', value: data.void_reason || 'Not specified' },
                        { label: 'AUTHORIZED BY', value: data.staff_name || 'System' }
                    ],
                    amountBox: {
                        label: 'VOIDED PORTFOLIO VALUE',
                        amount: data.amount || 0,
                        currency: data.currency || 'QAR',
                        subtext: 'Transaction Cancelled / Voided'
                    },
                    timestamp: safeFormatDate(new Date(), 'HH:mm:ss dd/MM/yyyy')
                });
                ...
                html = buildBoxedEmailHtml({
                    ...
                    dataFields: [
                        ...
                        { label: 'PROPERTY CONTEXT', value: property?.name || 'N/A' },
                        { label: 'FACILITY CONTEXT', value: outlet?.name || 'N/A' },
                        { label: 'SALES REPRESENTATIVE', value: data.staff_name || 'System Administrator' }
                    ],
                    amountBox: {
                        label: 'TOTAL CONTRIBUTION PAID',
                        amount: data.amount || 0,
                        currency: data.currency || 'QAR',
                        subtext: 'Direct Registration'
                    },
                    timestamp: safeFormatDate(new Date(), 'HH:mm:ss dd/MM/yyyy')
                });
            }

...
import { Building2, ReceiptText, Settings2, Check, X, ShieldCheck, FileText, Printer, FileDown, Globe } from 'lucide-react';
...
const Reports = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setCategories(cats);
      const startOfReport = startOfMonth(parseISO(reportMonth + '-01'));
  ...
  const activeColumns = useMemo(() => ALL_POSSIBLE_COLUMNS.filter(col => visibleColumnKeys.includes(col.key)), [visibleColumnKeys]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-sm no-print border border-slate-200 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-black text-slate-900 leading-none">Financial Ledger</h2>
          ...
          <input 
            ...
            className="h-10 px-3 rounded-lg border border-slate-200 font-bold text-xs uppercase bg-slate-50 outline-none" 
          />
        </div>

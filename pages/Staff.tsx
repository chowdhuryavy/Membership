    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      {showForm && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300 overflow-y-auto">
          <Card className="w-full max-w-4xl rounded-[3rem] border-slate-200 shadow-2xl overflow-hidden bg-white animate-in zoom-in-95 duration-300 my-8 max-h-[90vh] flex flex-col">
              ...
              <form onSubmit={handleSubmit} className="space-y-12">
                <div className="space-y-6">
                  <div className="flex items-center gap-4 mb-2">
                    <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xs shadow-sm">01</div>
                    <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Personnel Identity</h4>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50/30 p-8 rounded-[2.5rem] border border-slate-100">
                    <Input label="Full Identity Name *" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required className="h-14 rounded-2xl font-bold bg-white" />
                    <Input label="Professional Role *" value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} required className="h-14 rounded-2xl font-bold bg-white" placeholder="e.g. Senior Therapist" />
                    <Input label="Email Address" type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="h-14 rounded-2xl bg-white" />
                    <Input label="Contact Number" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="h-14 rounded-2xl bg-white" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    <div className="flex items-center gap-4 mb-2">
                      <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xs shadow-sm">02</div>
                      <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Performance Config</h4>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-4 mb-2">
                      <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xs shadow-sm">03</div>
                      <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Life Cycle</h4>
                    </div>
                    <div className="bg-slate-50/30 p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Onboarding Date</label>
                        <div className="relative group transition-all">
                          <Input 
                            ...
                            required 
                            className="h-14 rounded-2xl font-bold pl-12 border-slate-200 bg-white focus:bg-indigo-50/5 focus:border-indigo-600 transition-all shadow-sm" 
                          />
                          <div className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-500/60 group-focus-within:text-indigo-600 transition-colors">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between px-1 mb-1 flex-wrap gap-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Departure Management</label>
                          {(formData.inactive_date || !formData.is_active) && (
                             <span className={`text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${(!formData.inactive_date || formData.inactive_date <= format(new Date(), 'yyyy-MM-dd')) ? 'bg-red-100 text-red-600' : 'bg-amber-100 text-amber-600 font-bold'} border ${(!formData.inactive_date || formData.inactive_date <= format(new Date(), 'yyyy-MM-dd')) ? 'border-red-200' : 'border-amber-200'} shadow-sm animate-in fade-in zoom-in-95`}>
                               {(!formData.inactive_date || formData.inactive_date <= format(new Date(), 'yyyy-MM-dd')) ? 'Inactive Since' : 'Exit Scheduled'}
                             </span>
                          )}
                        </div>
                        ...
                        {(!formData.inactive_date && formData.is_active) ? (
                          ...
                          <div className="relative group animate-in slide-in-from-top-2 duration-300">
                            <Input 
                              ...
                              className={`h-14 rounded-2xl font-black pl-12 pr-12 border-rose-100 bg-white focus:border-rose-600 transition-all shadow-sm ${!formData.is_active && !formData.inactive_date ? 'border-red-500 ring-1 ring-red-500' : ''}`} 
                            />
                            ...
                            {formData.is_active && (
                              <button 
                                ...
                                className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 bg-white border border-slate-100 rounded-lg text-slate-400 hover:text-rose-600 hover:border-rose-100 shadow-sm transition-all z-10"
                                title="Cancel Exit"
                              >
                                <X className="w-4 h-4" />
                              </button>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 mb-2">
                    <div className="w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black text-xs shadow-sm">04</div>
                    <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Security & Deployment</h4>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="bg-slate-50/30 p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Assigned Facilities</label>
                      <div className="grid grid-cols-1 gap-2">
                        {allowedOutletsInProperty.map(outlet => (
                          <label key={outlet.id} className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-slate-200 cursor-pointer hover:border-indigo-300 hover:bg-indigo-50/10 transition-all group">
                            ...
                            <span className="text-xs font-black text-slate-700 uppercase tracking-tight group-hover:text-indigo-600 transition-colors">{outlet.name}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50/30 p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
                      ...
                      <div className="flex bg-slate-200/50 p-1.5 rounded-[1.8rem] border border-slate-100">
                        <button type="button" onClick={() => setFormData({ ...formData, is_active: true })} className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${formData.is_active ? 'bg-white text-emerald-600 shadow-xl shadow-emerald-500/10 border border-emerald-50' : 'text-slate-400 hover:text-slate-500'}`}>
                          <UserCheck className={`w-4 h-4 ${formData.is_active ? 'opacity-100' : 'opacity-40'}`} />
                          Active
                        </button>

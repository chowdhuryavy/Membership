  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Retention Focus */}
        <Card className="lg:col-span-2 border-slate-200/60 shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all duration-500 rounded-[2rem] overflow-hidden group h-fit">
        ...
        {/* Unified Financial & Trend Column */}
        <div className="space-y-6 flex flex-col">
            <Card className="border-slate-200/60 rounded-[2rem] shadow-sm hover:shadow-lg transition-all duration-500 bg-white overflow-hidden">
            ...
            <Card className="border-slate-200/60 rounded-[2rem] shadow-sm hover:shadow-lg transition-all duration-500 bg-white overflow-hidden">
                <CardContent className="p-8">
                    <div className="flex items-center justify-between mb-4">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Recent Enrollment Trend</p>
                        <TrendingUp className="w-4 h-4 text-emerald-500" />
                    </div>
                    <div className="flex items-end justify-between h-20 gap-2">
                        {enrollmentTrend.map((count, i) => (
                            <div key={i} className="flex-1 bg-indigo-50 hover:bg-indigo-600 rounded-t-lg transition-all duration-500 group relative" style={{ height: `${Math.max(20, count * 20)}%` }}>
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-slate-900 text-white text-[8px] font-black py-1 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20">
                                    {count} New
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="flex justify-between mt-3 px-1">
                        {Array.from({ length: 7 }).map((_, i) => (
                            <span key={i} className="text-[8px] font-bold text-slate-400 uppercase">
            ...
            <Card className="border-slate-200/60 rounded-[2rem] hover:shadow-lg transition-all duration-500 hover:border-indigo-100/50 group flex-1">
              <CardContent className="p-8">

    ...
    return (
        <Card className="max-w-2xl mx-auto rounded-[2rem] shadow-2xl overflow-hidden border-slate-200/60 bg-white">
            <CardHeader className="bg-indigo-600 text-white p-6 relative">
                <CardTitle className="text-xl font-black uppercase tracking-tight">Point of Sale Terminal</CardTitle>
    ...
    return (
        <div className="space-y-6">
            ...
            <ConfirmationModal 
                ...
                onConfirm={async () => { if (itemToDelete) { await db.deleteInventoryItem(itemToDelete); onRefresh(); } }} 
...
const Sales = () => {
    ...
    // Unify bookings and sales for the ledger
    ...
    const stats = useMemo(() => {
        ...
        const dayRetail = dayEntries.filter(e => e.category === 'Retail').reduce((acc, e) => acc + e.amount, 0);

        // MTD persistent stats
    ...
    const canManageInventory = user && hasPermission(user.role_id, 'inventory:manage');
    ...
    if (!canView) return (
        <div className="flex items-center justify-center h-96">
            <Card className="max-w-md text-center p-8 rounded-[2rem] border-red-100 bg-red-50/30">
                ...
                <p className="text-slate-500 mt-2 text-sm">Security clearance insufficient to view financial recognition ledgers.</p>
            </Card>
        </div>
    );
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
                    ...
                    <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm w-full sm:w-auto">
                        ...
                        <div className="relative flex items-center gap-2 px-2">
                             ...
                             <input 
                                ...
                                className="h-8 border-none outline-none font-black text-[10px] uppercase bg-transparent w-32 cursor-pointer" 
                             />
                        </div>
                    ...
                    <div className="flex bg-slate-200 p-1 rounded-xl border border-slate-300 shadow-inner w-full sm:w-auto">
                        ...
                        {canManageInventory && (
                            <button onClick={() => setActiveTab('inventory')} className={`flex-1 px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'inventory' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Inventory</button>
                        )}
                    </div>

                    {canCreate && (
                        <Button onClick={() => setShowForm(true)} className="w-full sm:w-auto rounded-xl h-11 px-6 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-100">
            ...
            {activeTab === 'ledger' ? (
                <>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-center bg-white p-4 rounded-[2rem] border border-slate-200/60 shadow-sm">
                         <div className="relative group flex-1 w-full">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within:text-indigo-600 transition-colors" />
                            <input 
                                placeholder="Filter day records by guest or item..." 
                                className="w-full h-11 pl-11 pr-4 rounded-xl bg-slate-50 border border-slate-200 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all text-sm font-bold"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                            />
                        </div>
                        <Select 
                            options={['All', 'Retail', 'Personal Training', 'Entrance Fee', 'Massage', 'Other'].map(c => ({ value: c, label: c }))} 
                            value={categoryFilter}
                            onChange={e => setCategoryFilter(e.target.value)}
                            className="h-11 w-full sm:w-48 rounded-xl font-bold text-xs"
                        />
                    </div>
                    ...
                    <Card className="rounded-[2.5rem] border-slate-200/60 shadow-xl overflow-hidden bg-white">
                        <div className="px-8 py-5 bg-slate-50/50 border-b flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-[11px] font-black uppercase tracking-[0.1em] text-slate-900">{format(selectedDate, 'EEEE, dd MMMM yyyy')}</h4>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="flex flex-col items-end">
                                    <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Recognized Events</span>
                                    <span className="text-xs font-black text-slate-700">{filteredEntries.length} Records</span>
                                </div>
                                <div className="h-8 w-px bg-slate-200"></div>
                                <div className="flex flex-col items-end">
                                    <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Yield Captured</span>
                                    <span className="text-sm font-black text-indigo-600 tracking-tighter">{formatMoney(stats.dayTotal)}</span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    {filteredEntries.map(entry => (
                                        <tr key={entry.id} className="hover:bg-indigo-50/20 transition-colors group">
                                            ...
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-7 h-7 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-[9px] uppercase">{entry.guest_name.charAt(0)}</div>
                                                    <span className="font-black text-slate-700 tracking-tight uppercase text-[11px]">{entry.guest_name}</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5">
                                            ...
                                            <td className="px-8 py-5 text-right font-black text-slate-900 tabular-nums text-[11px]">
                                                {formatMoney(entry.amount)}
                                            </td>
                                            <td className="px-8 py-5 text-center">
                                                <span className={`inline-flex px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${entry.type === 'booking' ? 'bg-purple-50 text-purple-600 border-purple-100' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                                                    {entry.method}
                                                </span>
                                            </td>
                                            <td className="px-8 py-5 text-right">
                                                {entry.type === 'pos' && canDelete && (
                                                    <button 
                                                ...
                                                {entry.type === 'booking' && (
                                                    <div className="text-[8px] font-black text-slate-300 uppercase tracking-widest">Locked (Svc)</div>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
                                    {filteredEntries.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="px-8 py-20 text-center">
                                                ...
                                                <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Zero recognition events for this date scope.</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
            ...
            <ConfirmationModal 
                ...
                onConfirm={async () => {
                    if (itemToDelete) {
                        await db.deleteSale(itemToDelete);
                        loadData();
                    }
                }}
                title="Void Recognition"
                description="Are you sure you want to void this transaction? This will reverse the revenue recognition and return any inventory assets to stock."

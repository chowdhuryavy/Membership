...
import { WhatsAppConfigModule } from '../components/whatsapp/WhatsAppConfigModule';
...
const SettingsPage = () => {
  ...
  const groupedIncentives = useMemo(() => {
      return incentiveRules.reduce((acc, rule) => {
          const property = rule.scope === 'Global' ? 'Global' : (rule.scope === 'Property' ? properties.find(p => p.id === rule.scope_id)?.name || 'Unknown' : (outlets.find(o => o.id === rule.scope_id)?.property_id ? properties.find(p => p.id === outlets.find(o => o.id === rule.scope_id)?.property_id)?.name : 'Unknown'));
          const outlet = rule.scope === 'Global' ? 'Global' : (rule.scope === 'Property' ? 'All Outlets' : (outlets.find(o => o.id === rule.scope_id)?.name || 'Unknown'));
          const type = rule.applies_to;
          if (!acc[property]) acc[property] = {};
          if (!acc[property][outlet]) acc[property][outlet] = {};
          if (!acc[property][outlet][type]) acc[property][outlet][type] = [];
          acc[property][outlet][type].push(rule);
          return acc;
      }, {} as Record<string, Record<string, Record<string, IncentiveRule[]>>>);
  ...
  useEffect(() => {
      ...
      if (propertiesList.length > 0 && (!activeProperty || !propertiesList.includes(activeProperty))) {
  ...
  const handleSendTestReport = async (recipient: ReportRecipient) => {
    try {
      ...
      if (['members_joined', 'member_freeze', 'sale_void'].includes(recipient.report_type)) {
        ...
        const outId = recipient.outlet_id || (outlet && typeof outlet === 'object' ? outlet.id : 'all');
    ...
    <div className="space-y-10 max-w-7xl mx-auto animate-in fade-in duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          {activeTab === 'company' && (
              ...
              {activeTab === 'incentives' && (
                  <Card className="rounded-[3.5rem] border-slate-200/60 shadow-xl overflow-hidden min-h-[600px] flex flex-col bg-white">
                      <CardHeader className="bg-slate-50 p-8 border-b border-slate-100 flex items-center justify-between">
                          ...
                          <Button onClick={() => { setEditingId(null); setIncentiveForm({ name: '', scope: 'Global', scope_id: 'global', applies_to: 'Massage', target_id: 'all', distribution_type: 'Individual', calculation_type: 'Percentage', value: 0, min_price: 0, max_price: 99999, min_duration_minutes: 0, max_duration_minutes: 999, apply_discount_percentage: true, disable_shared_incentive: false, is_active: true }); setShowForm(true); }} className="h-14 px-8 rounded-2xl font-black text-xs uppercase"><Plus className="w-5 h-5 mr-2" /> Authorize Logic</Button>
                      </CardHeader>
                      <CardContent className="p-0 flex-1 overflow-hidden">
                          {Object.keys(groupedIncentives).length > 0 ? (
                              ...
                              <div className="p-10 text-center text-slate-400 font-black uppercase text-xs tracking-widest">No contract logic authorized yet.</div>
                          )}
                      </CardContent>
                  </Card>
              )}
              ...
              {(activeTab === 'whatsapp_config' || activeTab === 'whatsapp_rules' || activeTab === 'whatsapp_templates' || activeTab === 'whatsapp_settings') && (
                <WhatsAppConfigModule
                  ...
                  waConfig={waConfig}
                  isLoadingWaData={isLoadingWaData}
                  onToggleWaRule={handleToggleWaRule}
                  onSaveWaRule={handleSaveWaRule}
                  onDeleteRule={handleDeleteWaRule}
                  onTestWaRule={handleTestWaRule}
                  onSaveWaTemplate={handleSaveWaTemplate}
                  onDeleteWaTemplate={handleDeleteWaTemplate}
                  onSaveWaConfig={handleSaveWaConfig}
                  onTestWaConnection={handleTestWaConnection}
                  activeWaCompany={activeWaCompany}
                  activeWaProperty={activeWaProperty}
                  activeWaOutlet={activeWaOutlet}
                />
              )}
          </div>
                                          ...
                                          <div className="space-y-4 animate-in slide-in-from-top-1">
                                              <div className="space-y-1.5">
                                                    ...
                                                    <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 h-10">
                                                        ...
                                                        <button type="button" onClick={() => setIncentiveForm({...incentiveForm, referral_payee: 'Referrer'})} className={`flex-1 rounded-lg text-[9px] font-black uppercase transition-all flex items-center justify-center gap-2 ${incentiveForm.referral_payee === 'Referrer' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400'}`}>Referrer</button>
                                                        <button type="button" onClick={() => setIncentiveForm({...incentiveForm, referral_payee: 'Both'})} className={`flex-1 rounded-lg text-[9px] font-black uppercase transition-all flex items-center justify-center gap-2 ${incentiveForm.referral_payee === 'Both' ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400'}`}>Both</button>
                                                    </div>
                                              </div>
                                              <div className="flex items-center gap-2">
                                                  <input 
